...
using FairlyInspectionNotify.Models.Others;
using System;
...
namespace FairlyInspectionNotify.Repositories
{
    public class GenericRepository<TEntity> : IRepository<TEntity> where TEntity : class
    {
        ...
        private TEntity setInsertData(TEntity entity)
        {
            ...
            {
                if (p.Name == nameof(EntityBase.UpdateTime) || p.Name == nameof(EntityBase.CreateTime))
                {
                    p.SetValue(entity, DateTime.Now);
                }
                if (p.Name == nameof(EntityBase.UpdateDate) || p.Name == nameof(EntityBase.CreateDate))
                {
                    p.SetValue(entity, DateTime.Now);
                }
        ...
        private TEntity setUpdateData(TEntity entity, bool isGuid = false)
        {
            ...
            foreach (var p in props)
            {
                ...
                else if (p.Name == nameof(EntityBase.Creater) || p.Name == nameof(EntityBase.CreateTime) || p.Name == nameof(EntityBase.CreateDate))
                {
                    //not handle
                }
                else
